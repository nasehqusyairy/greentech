    ...
    <div class="card-body">
      ...
      <div class="mb-3">
        ...
        <select name="attendance" id="attendance" class="form-select">
          <option value="offline" <?= old('attendance') == 'offline' ? 'selected' : ''; ?>>Offline</option>
      ...
      <div class="mb-3">
        ...
        <select name="ttype_id" id="ttype_id" class="form-select">
          <?php foreach ($ttypes as $ttype) : ?>
            <option value="<?= $ttype->id ?>" <?= old('ttype_id') == $ttype->id ? 'selected' : ''; ?>><?= $ttype->name ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="mb-3">
        ...
        <select name="trole_id" id="trole_id" class="form-select">
          <?php foreach ($troles as $trole) : ?>
            <option value="<?= $trole->id ?>" <?= old('trole_id') == $trole->id ? 'selected' : ''; ?>><?= $trole->name ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="mb-3">
